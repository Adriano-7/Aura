...
@if (session('status'))
    ...
    @if(Auth::check() && (Auth::user()->participatesInEvent($event) || Auth::user()->organizations->contains($event->organization_id) || Auth::user()->is_admin))
    <div class="container navSect" id="polls">
        ...
        <div style="display: flex; align-items: center;">
            @if(Auth::check() && Auth::user()->organizations->contains($event->organization_id))
                <button type="button" class="btn" data-toggle="modal" data-target="#createPollModal"><i class="bi bi-plus" style="color: white; font-weight: bold; font-size:1.5em; margin-left: 10px;"></i></button>
            @endif
        </div>
