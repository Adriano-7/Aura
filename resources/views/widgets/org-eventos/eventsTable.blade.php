        ...
        <div class="card mx-auto">
            ...
            <div class="row event">
                ...
                <div class="col-md-7" onclick="window.location.href = '/evento/{{ $event->id }}'"
                    ...
                    !$event->participants()->get()->contains(Auth::user()) && $event->start_date > now())
                    <div class="col-md-2 ml-auto">
                        <button type="button" class="join-event" id="button-{{$event->id}}"
                            onclick="joinEvent({{$event->id}})">Aderir ao Evento</button>
                    </div>
                    @elseif (Auth::check() && !Auth::user()->is_admin &&
                    $event->participants()->get()->contains(Auth::user()) && $event->start_date > now())
                    <div class="col-md-2 ml-auto">
                        <button type="button" class="leave-event" id="button-{{$event->id}}"
                            onclick="leaveEvent({{$event->id}})">Sair do Evento</button>
                    </div>
                    @endif
                </div>
                @endforeach
