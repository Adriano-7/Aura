@if($event->polls->isNotEmpty())
    @foreach($event->polls as $poll)
        <div class="card poll-card" data-poll-id="{{ $poll->id }}" data-is-organizer-or-admin="{{ Auth::check() && (Auth::user()->userOrganizations->contains($event->organization_id) || Auth::user()->is_admin) ? 'true' : 'false' }}">
